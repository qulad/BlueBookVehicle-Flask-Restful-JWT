...
class AdminAcceptPendingEditListingResource(Resource):
    """
    AdminAcceptPendingEditListingsResource is used by admins to approve edited listing are waiting for approval.
    ...
    Request constrains:
        ...
        listing_id must be linked to an existing listing that is waiting for edit approval in the database.
    Responses:
        ...
        listing_id is not linked to any listing:
            ...
            Response code: 404
        listing is not waiting for edit approval:
    ...
    def post(self):
        ...
        waiting_for_approval = listing.edit_waiting
        if not waiting_for_approval:
            return {"message": "Listing is already approved."}, 403
        
        listing.edit_waiting = False
        listing.vehicle_id = listing.new_vehicle_id
        listing.edited = True
        db.session.commit()
